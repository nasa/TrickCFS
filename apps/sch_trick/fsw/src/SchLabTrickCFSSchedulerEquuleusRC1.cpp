...
/*
 * SchLabTrickCFSSchedulerEquuleusRC1.cpp
 *
 *  Created on: May 22, 2024
 *      Author: tbrain
 */

#include "SchLabTrickCFSSchedulerEquuleusRC1.hh"
...
SchLabTrickCFSSchedulerEquuleusRC1::SchLabTrickCFSSchedulerEquuleusRC1()
    : BaseTrickCFSScheduler("SchLabTrickCFSSchedulerEquuleusRC1"),
      TblHandle(0),
      schedRateInMicros(0),
      schedFreq(0)
{
...
size_t SchLabTrickCFSSchedulerEquuleusRC1::initTables()
{
    int i, x;
    CFE_Status_t Status;
    uint32 TimerPeriod;
     ...
     */
    Status = CFE_TBL_Register(&TblHandle, "SCH_LAB_SchTbl", sizeof(SCH_LAB_ScheduleTable_t), CFE_TBL_OPT_DEFAULT, NULL);
    ...
    else
    {
         ...
         ** Loading Table
         */
        Status = CFE_TBL_Load(TblHandle, CFE_TBL_SRC_FILE, SCH_TBL_DEFAULT_FILE);
        if(Status != CFE_SUCCESS)
        {
            message_publish(MSG_ERROR,
...
double SchLabTrickCFSSchedulerEquuleusRC1::getMinorFrameRate()
{
    return static_cast<double>(schedRateInMicros) / 1.0E6;
}

size_t SchLabTrickCFSSchedulerEquuleusRC1::getMinorFrameMsTics()
{
    return schedRateInMicros / 1E3;
}

size_t SchLabTrickCFSSchedulerEquuleusRC1::processScheduleTable()
{
    /*
...
void SchLabTrickCFSSchedulerEquuleusRC1::addScheduledCmd(size_t mid,
                                                         double cmdRate,
                                                         size_t cmdCode,
