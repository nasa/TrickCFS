...
def parsexmlFile(xmloutputfile):
   testFailed = True
   ntests = 1
   nfailures = 1
   if os.path.isfile(xmloutputfile):
     doc = ET.parse(xmloutputfile)
     root = doc.getroot()
     ntests = int(root.attrib["tests"])
     nfailures = int(root.attrib["failures"])
     if nfailures == 0:
       testFailed = False
   else:
     errormsg = "File doesn't exist - " + xmloutputfile
     outMsg(errormsg)

   return testFailed, ntests, nfailures

def getGccMajVer():
   gccVer = None
   gcmd = 'gcc --version'
   proc = subprocess.Popen(gcmd.split(), stdout=subprocess.PIPE, encoding="utf-8")
   res = proc.communicate()
   gccVerLin = res[0].split('\n')[0]
   for c in gccVerLin:
     if c.isdigit():
        if gccVer == None:
           gccVer = c
